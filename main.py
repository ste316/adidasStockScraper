...
class adidasStockScraper:
    ...
    def getPageOf(self, pid: str):
        link = f'https://{self.domain}/ste316/{pid}.html'
        return link
    ...
    def run(self):
        adilette = ['GX6949','GX6947','GX6950','GY1597','GX6945','GX6946'] # hard coded product id
        ...
        while True:
            for pid in adilette:
                ...
                print(f'pid: {res["id"]} total stock: {totalStock}\n{toPrint}{self.getPageOf(pid)}\n')
            if self.loop == False:
                break
            sleep(self.timeout)
